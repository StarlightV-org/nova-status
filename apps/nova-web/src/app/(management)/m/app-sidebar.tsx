	...
	return (
		<Collapsible open={open} onOpenChange={setOpen} className="border-b border-sidebar-border last:border-b-0">
			...
			<CollapsibleContent className="flex flex-col gap-1 px-2 pb-2">
				{monitors.length === 0 ? (
					<p className="px-1 text-xs text-muted-foreground">No monitors</p>
				) : (
	...
	return (
		<div className="flex h-full flex-col bg-sidebar text-sidebar-foreground">
			<div className="flex items-center justify-between gap-2 border-b border-sidebar-border px-3 py-3">
				<h2 className="text-sm font-semibold text-sidebar-foreground">Monitors</h2>
				<CreateMonitorDialog />
			</div>
			...
			<div className="min-h-0 flex-1 overflow-auto">
				...
				{ungrouped.length > 0 && <SidebarGroupSection label="None" monitors={ungrouped} />}
			</div>
		</div>
	);
}
