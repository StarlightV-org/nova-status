...
export async function uptimeForMonitor(database: typeof DbClient, monitorId: string): Promise<MonitorUptime> {
...
export async function uptimeForMonitors(
	...
	const rows = await database
		...
		.from(monitorStatus)
		.where(inArray(monitorStatus.monitorId, monitorIds))
